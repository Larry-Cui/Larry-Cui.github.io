...
$c, c'$ stand for some constant numbers. We can see that any terms before $r^m$ is dividable by $n$, and $r^m \equiv r^m \;(\mathrm{mod}\; n)$.
...
> Let $a$ and $n$ be positive integers with gcd$(a,n) = 1$, then $$ a^{\phi(n)} \equiv 1 \;(\mathrm{mod}\; n)$$
