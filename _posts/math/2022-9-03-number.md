...
**Corollary 1 proof:** from $d=\text{gcd}(x,y)$, we get $$ 1=\text{gcd}\left(\frac{x}{d}, \frac{y}{d}\right)$$

Let $a=x/d$ and $n=y/d$, we rewrite equation as gcd$(a,n)=1$, and the corollary becomes
...
We know that the the multiple of all element from set A and multiple of all elements from residue set have the same remainder if divided by $p$, as stated below:
...
> Let $a$ and $n$ be positive integers with gcd$(a,n) = 1$, then $$ a^{\phi(n)} \equiv 1 \;(\bmod\; n)$$
...
As a result, $\{ ar_1, ar_2, \dots ar_{\phi(n)} \}$ is also a reduced residue system modulo $n$.

We now have the following conclusion:

$$
\begin{align*}
ar_1 ar_2 \cdots ar_{\phi(n)} &\equiv r_1 r_2 \cdots r_{\phi(n)}  \;(\bmod\; n) \\
a^{\phi(n)} r_1 r_2 \cdots r_{\phi(n)} &\equiv r_1 r_2 \cdots r_{\phi(n)} \;(\bmod\; n) \\
