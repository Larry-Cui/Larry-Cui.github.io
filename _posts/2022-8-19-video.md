...
- [Video from Youtube](#video-from-youtube)
  - [Example: Time Laspe of the Harbour View](#example-time-laspe-of-the-harbour-view)
...
Apparently, we also need the video window sourced from youtube to be responsive, following the logic from above, you may think it should be like,
...
But, if we go with above wordings, we will end up with a video window with a fixed height of 150px. We need a little bit trick to solve this issue.
...
The key to this solution is the `padding-bottom` of the `videoWrapper`. 56.25% means the content bottom of `videoWrapper` is 56.25% of the width from the bottom border of the `videoWrapper` element, which makes the bottom padding area exactly a 16:9 ratio. So if we set the ratio at 50%, the padding area is a 2:1 rectangler.
...
<br>
### Example: Time Laspe of the Harbour View
